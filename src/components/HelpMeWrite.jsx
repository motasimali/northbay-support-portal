...
export default function HelpMeWrite({
  ...
  const { generate, loading, error: apiError } = useOpenAI();

  const charCount = notes.trim().length;
  const disabledGenerate = loading || charCount < 20;
      ...
      <div className="flex gap-2">
        ...
        {!suggestion ? (
          <LoadingButton
            ...
            className="bg-black text-white cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="space-y-4">
        ...
        <label className="block">
          <span className="text-sm text-gray-700">
            {t("yourNotes", { defaultValue: "Your notes (min 20 chars)" })}
          </span>
          <textarea
